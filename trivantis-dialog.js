...
jsDialog.prototype.create = function(doc)
{
    ...
    if(!is.isMobile.any()){
		screenWidth = getDisplayDocument().getElementById('pageDIV').style.width;//g.scrollWidth || w.innerWidth  || e.clientWidth  || g.clientWidth || g.scrollWidth;
    }else{
		screenWidth = w.innerWidth  || e.clientWidth  || g.clientWidth || g.scrollWidth;
	}
	...
	this.divEle.style.height = this.isAutoRsz ? '' : (this.height - this.titleHt - this.titleHtAdj) + "px";
  if(!is.isMobile.any()){
	this.divEle.style.transform = d.getElementById('pageDIV').style.transform;
  }
...
jsDialog.prototype.setInitWinPos = function()
{
	...
	this.yPos = top;
if(!is.isMobile.any()){
   	//this.divEle.style.left = $("#pageDIV").css("left");// left + 'px';
	if(window.innerWidth>iScaleMaxWidth && calcZooming()==1)	{
		setTimeout(function(){
		var pgElem=$("#pageDIV");
...
jsDlgBox.prototype.create = function(doc)
{
	...
	setTimeout( function(){
		...
		aria.Utils.focusFirstDescendant(ele, true);
			createPreloderElement(document.getElementById(THIS.divEleInner.id)); 
	}, 250);
